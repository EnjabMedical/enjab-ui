...
- ALWAYS ship the favicon via `@enjab-ui/favicon`: it adds `app/icon.tsx` + `app/apple-icon.tsx`, code-generated PNG icons (the Enjab gradient square + a glyph) that render in Safari, so no separate `.ico` is needed. Brand a tool by editing the `GLYPH` constant (a different letter, or any Lucide icon, same gradient/shape). Default is `E`. Delete any old static `app/icon.svg` so it doesn't clash.
